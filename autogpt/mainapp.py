...
logger.speak_mode = speak
    ...
    @app.post("/aaia_message")
    async def get_aaia_message(message: Message):
        ...
        if user_id not in agents:
            ...
            print('Buscar usuario en DB')
            # Si está:
            if 'si' == 'no':
                if len(agents) == agents_cache_limit:
                    # Eliminamos el agente con interacción más antigua
                    print('Eliminamos el agente con la interacción más antigua')
        ...
        try:
            if start_key:
                aaia_message = agents[user_id].start_agent()
            else:
                aaia_message = agents[user_id].generate_answer(question)
